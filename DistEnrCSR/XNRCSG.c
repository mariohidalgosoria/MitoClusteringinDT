...
#include<math.h>
#include<stdlib.h>
#define Nds 41 //number of rows in each file  id.dat
...
int main(void){
  ...
  for(id=ini;id<=Nexp;id++){          
    ...
    i=0;
    //Captura de datos:
    sprintf(nom,"XNRCSGT%d.dat",id);//coloca el id, xid.dat en una cadena 'nom'
  ...
  sprintf(nom,"EXNRCSG.dat");//la cadena nom='x_mean.dat'
